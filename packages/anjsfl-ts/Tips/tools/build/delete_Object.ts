...
const CODE_TO_COMMENT = 'Object.defineProperty(exports, "__esModule", ({ value: true }));';
...
function processFiles(): void {
    ...
    if (jsflFiles.length === 0) {
        console.log(`未在 "${TARGET_DIR}" 及其子文件夹中找到任何 .${TARGET_EXTENSION} 文件。`);
        return;
    }
        ...
        try {
            ...
            if (content.includes(CODE_TO_COMMENT)) {
                ...
                if (newContent !== content) {
                    ...
                    console.log(`[跳过] ${relativePath} (包含文本但未匹配到独立行，或已被注释)`);
                    skipCount++;
                }
            } else {
                // 完全不包含该字符串
