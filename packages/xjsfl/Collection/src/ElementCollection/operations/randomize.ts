...
type SingleProperty = 'x' | 'y' | 'width' | 'height' | 'rotation' |
...
export function randomize(this: ElementCollection, prop: Record<string, Modifier>): ElementCollection;
export function randomize(this: ElementCollection, prop: Property, modifier?: Modifier): ElementCollection ;

export function randomize(this: ElementCollection, prop: Property | Record<string, Modifier>, modifier?: Modifier) {
...
function randomizeSingleProperty(prop: SingleProperty, modifier?: Modifier) {
    ...
    for (const element of self_elements) {
        ...
        element[prop as keyof FlashElement] = isArray
            ? Utils.randomValue(modifier as [number, number])
            : Utils.randomizeValue(value, modifier)
    }
}
...
function randomizePosition(modifier?: Modifier) {
    ...
    const values = Array.isArray(modifier)
        ? modifier
        : [modifier, modifier]
...
function randomizeScaleOrSize(prop: string, modifier?: Modifier) {
    ...
    for (const element of self_elements) {
        ...
        if (Array.isArray(modifier)) {
            ...
            const value = Utils.randomizeValue(element[px], modifier)
            values = [value, value]

        } else {

            const max = Math.max(element[px], element[py])
            const value = Utils.randomizeValue(max, modifier)
            values = [value, value]
