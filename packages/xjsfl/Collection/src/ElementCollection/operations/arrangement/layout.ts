...
export function layout(
    ...
    columns: number = 0, // Number of columns (defaults to element count)
    ...
    const startX = self_elements[0].x; // Initial x-position based on the first element
    const startY = self_elements[0].y; // Initial y-position based on the first element

    for (let i = 0; i < self_elements.length; i++) {
        // Loop through all elements
        const element = self_elements[i];
