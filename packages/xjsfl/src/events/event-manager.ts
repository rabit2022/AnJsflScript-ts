    ...
    add(
        ...
        if (handler.callbacks.size === 1 && this.config.eventListener) {
            try {
                ...
                if (this.logger?.warn) {
                    this.logger.warn(`Failed to register event listener for ${type}:`, error);
                }
            }
        }
    ...
    private createGatewayHandler(type: string): Function {
        return () => {
            try {
                ...
                if (this.logger?.warn) {
                    this.logger.warn(`Error in event gateway handler for ${type}:`, error);
                }
            }
        };
    }
    ...
    private removeSystemListener(type: string): void {
        ...
        if (handler?.id && this.config.eventListener) {
            try {
                ...
                if (this.logger?.warn) {
                    this.logger.warn(`Failed to remove system listener for ${type}:`, error);
                }
            }
        }
    }
