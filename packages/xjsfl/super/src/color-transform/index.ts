...
export const COLOR_TRANSFORM_BUILDERS = {
    ...
    // 动态访问方法
    get(type: string): any {
        return COLOR_TRANSFORM_BUILDERS_MAP[
            type as keyof typeof COLOR_TRANSFORM_BUILDERS_MAP
        ];
    }
};
...
export const COLOR_TRANSFORM_INSTANCES = {
    ...
    // 动态访问方法
    get(type: string): any {
        return COLOR_TRANSFORM_INSTANCES_MAP[
            type as keyof typeof COLOR_TRANSFORM_INSTANCES_MAP
        ];
    }
};
...
export function createColorTransform(type: string, config?: any) {
    const TransformClass =
        COLOR_TRANSFORMS_MAP[type as keyof typeof COLOR_TRANSFORMS_MAP];
    if (!TransformClass) {
...
export function createColorTransformBuilder(type: string) {
    const BuilderClass =
        COLOR_TRANSFORM_BUILDERS_MAP[type as keyof typeof COLOR_TRANSFORM_BUILDERS_MAP];
    if (!BuilderClass) {
