...
export class ElementCollection extends Array<FlashElement> {
     ...
     * @param element - Optional parameter to specify which element(s) to select:
     *                   - If `true`, selects all elements.
     *                   - If a `number`, selects the element at that index.
     *                   - If a `string`, selects the element with that name.
     *                   - If an `Element`, selects that specific element.
     * @returns The original ElementCollection object for chaining.
     */
    select(element?: boolean | number | string | Element): ElementCollection {
        ...
        if (element === true || element === undefined) {
            // Select all elements if `true` or no argument is provided
            this.dom.selection = self_elements;
        } else {

            // string
            // Find and select a single element based on the type of `element`
            const selectedElement = find(self_elements, element);
            this.dom.selection = selectedElement;
        }
