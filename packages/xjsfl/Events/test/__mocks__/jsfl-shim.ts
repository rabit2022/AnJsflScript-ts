...
export const fl = {
    ...
    removeEventListener(type: string, id: number): void {
        ...
        if (eventListeners[normalizedType]) {
            eventListeners[normalizedType] = eventListeners[normalizedType].filter(l => l.id !== id);
        }
        console.log(`[MOCK] removeEventListener: ${type}, id=${id}`);
    },

    // 👇 新增：测试用 —— 手动触发鼠标移动事件
    __triggerMouseMove(x: number, y: number, opts: { shift?: boolean; ctrl?: boolean; alt?: boolean } = {}) {
    ...
    outputPanel: {
        clear() {},
        trace(msg: string) { console.log(`[Output] ${msg}`); }
    }
};
